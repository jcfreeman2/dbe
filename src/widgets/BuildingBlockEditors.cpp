...
template<>
editor_data<dunedaq::config::relationship_t>::editor_data (
  dunedaq::config::relationship_t const & virtue )
  :
  ...
  CurrentItem ( nullptr )
{
  ...
  if ( ( Virtue.p_cardinality == dunedaq::config::zero_or_many ) or ( Virtue.p_cardinality
  ...
  if ( ( Virtue.p_cardinality == dunedaq::config::one_or_many ) or ( Virtue.p_cardinality
                                                                 == dunedaq::config::only_one ) )
  {
...
void relation::buildtooltip()
{
  ...
  setToolTip (
    ...
    + QString ( "             Cardinality:     %1 \n" ).arg (
      dunedaq::config::relationship_t::card2str ( Virtue.p_cardinality ) )
...
void stringattr::buildtooltip()
{
  ...
  setToolTip (
    QString ( "Attribute Name:           %1 \n" ).arg ( Virtue.p_name.c_str() ) + QString (
      "          Type:           %1 \n" ).arg (
      dunedaq::config::attribute_t::type2str ( Virtue.p_type ) )
    + QString ( "          Range:          %1 \n" ).arg ( Virtue.p_range.c_str() )
    + QString ( "          Format:         %1 \n" ).arg (
      dunedaq::config::attribute_t::format2str ( Virtue.p_int_format ) )
  ...
  this_native_base ( -1 )
{
  ...
  if ( ! ( Virtue.p_type == dunedaq::config::float_type
           || Virtue.p_type == dunedaq::config::double_type ) )
  {
    if ( Virtue.p_int_format == dunedaq::config::oct_int_format )
    {
      FormatBox->setCurrentIndex ( 2 );
      this_base = 8;
      this_native_base = 8;
    }
    else if ( Virtue.p_int_format == dunedaq::config::dec_int_format )
    {
      FormatBox->setCurrentIndex ( 0 );
      this_base = 10;
      this_native_base = 10;
    }
    else if ( Virtue.p_int_format == dunedaq::config::hex_int_format )
    {
...
bool numericattr::ValidateIntegerValue ( QString const & text )
{
  auto checkInteger = [this, text]()
  {
      ...
      if ( input.p_type == dunedaq::config::u8_type
           or input.p_type == dunedaq::config::u16_type
           or input.p_type == dunedaq::config::u32_type
           or input.p_type == dunedaq::config::u64_type )
      {
        ...
        else if ( input.p_type == dunedaq::config::u16_type )
        {
          min_u = 0;
          max_u = USHRT_MAX;
        }
        else if ( input.p_type == dunedaq::config::u32_type )
        {
          min_u = 0;
          max_u = ULONG_MAX;
        }
        else if ( input.p_type == dunedaq::config::u64_type )
        {
          min_u = 0;
      ...
      else
      {
        ...
        if ( input.p_type == dunedaq::config::s8_type )
        {
          min_s = SCHAR_MIN;
          max_s = SCHAR_MAX;
        }
        else if ( input.p_type == dunedaq::config::s16_type )
        {
          min_s = SHRT_MIN;
          max_s = SHRT_MAX;
        }
        else if ( input.p_type == dunedaq::config::s32_type )
        {
          min_s = LONG_MIN;
          max_s = LONG_MAX;
        }
        else if ( input.p_type == dunedaq::config::s64_type )
        {
...
void numericattr::AddToList()
{
  ...
  auto convert_to_proper_base = [this] (const QString& value) -> QString {
      if((value.isEmpty() == false) && (this_native_base != -1) && (this_native_base != this_base)) {
          ...
          bool Unsigned = ( virtue.p_type == dunedaq::config::u8_type
                              || virtue.p_type == dunedaq::config::u16_type
                              || virtue.p_type == dunedaq::config::u32_type
  ...
  t_virtue const & virtue = this_base_data_editor->get();
  if ( ( virtue.p_type == dunedaq::config::float_type
           or virtue.p_type == dunedaq::config::double_type ))
  {
  ...
  m_base_data_editor ( std::static_pointer_cast<t_build_block_editor> ( p_data_editor ) )
{
  ...
  if ( Virtue.p_type == dunedaq::config::class_type )
  {
...
void combo::SetEditor()
{
  ...
  if ( virt.p_type == dunedaq::config::bool_type )
  {
    QStringList tmp
    { "true", "false" };
    SetValidatorData ( tmp );
  }
  else if ( virt.p_type == dunedaq::config::enum_type )
  {
  ...
  else if ( virt.p_type == dunedaq::config::class_type )
  {
